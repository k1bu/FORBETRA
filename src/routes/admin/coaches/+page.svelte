...
<svelte:head>
	<title>Coach Assignments | Forbetra Admin</title>
</svelte:head>
...
<section class="mx-auto flex max-w-5xl flex-col gap-6 p-6">
	<header>
		...
		<p class="text-sm text-text-secondary">
			All coach-client relationships and pending invitations
		</p>
	</header>
		...
		{#if data.coachClients.length > 0}
			<div class="overflow-x-auto">
				<table class="min-w-full text-sm">
					<thead class="text-left text-xs font-semibold tracking-wide text-text-tertiary uppercase">
					...
					<tbody>
						{#each data.coachClients as rel (rel.id)}
							<tr class="border-b border-border-default hover:bg-surface-subtle">
								...
								<td class="px-3 py-2">
									<a
										href="/admin/users/{rel.individual.id}"
										class="font-medium text-accent hover:underline"
									>
								...
								<td class="px-3 py-2">
									{#if rel.archivedAt}
										<span
											class="rounded bg-surface-subtle px-2 py-0.5 text-xs font-semibold text-text-tertiary"
											>Archived {formatDate(rel.archivedAt)}</span
										>
									{:else}
										<span
											class="rounded bg-success-muted px-2 py-0.5 text-xs font-semibold text-success"
											>Active</span
										>
									{/if}
								</td>
							...
							<tr class="border-b border-border-default hover:bg-surface-subtle">
								...
								<td class="px-3 py-2">
									...
									{:else if invite.cancelledAt}
										<span
											class="rounded bg-error-muted px-2 py-0.5 text-xs font-semibold text-error"
											>Cancelled</span
										>
									{:else if new Date(invite.expiresAt) < new Date()}
										<span
											class="rounded bg-warning-muted px-2 py-0.5 text-xs font-semibold text-warning"
											>Expired</span
										>
									{:else}
										<span
											class="rounded bg-accent-muted px-2 py-0.5 text-xs font-semibold text-accent"
											>Pending</span
										>
									{/if}
								</td>
