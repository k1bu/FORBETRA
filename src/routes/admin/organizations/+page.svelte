...
<section class="mx-auto flex max-w-5xl flex-col gap-6 p-6">
	<header class="space-y-2">
		...
		<p class="text-text-secondary">
			Create organizations, manage members, and configure domain-based auto-assignment.
		</p>
	</header>
	...
	<div class="rounded-lg border border-border-default bg-surface-raised p-5">
		...
		<form method="post" action="?/createOrg" class="flex flex-wrap items-end gap-3">
			...
			<div class="min-w-[200px] flex-1">
				<label for="org-domain" class="mb-1 block text-xs font-medium text-text-tertiary"
					>Email Domain (optional)</label
				>
				<input
					id="org-domain"
					type="text"
		...
		<p class="mt-2 text-xs text-text-tertiary">
			If a domain is set, new users signing up with that email domain will automatically join this
			organization.
		</p>
	</div>
			...
			{#each data.organizations as org (org.id)}
				<div class="overflow-hidden rounded-lg border border-border-default bg-surface-raised">
					...
					<button
						...
						class="flex w-full items-center justify-between px-5 py-4 text-left transition-colors hover:bg-surface-subtle"
					>
						<div>
						...
						<svg
							class="h-5 w-5 text-text-tertiary transition-transform {expandedOrg === org.id
								? 'rotate-180'
								: ''}"
							fill="none"
							stroke="currentColor"
					...
					{#if expandedOrg === org.id}
						<div class="border-t border-border-default px-5 py-4">
							...
							<form method="post" action="?/addMember" class="mb-4 flex flex-wrap items-end gap-3">
								...
								<div class="min-w-[200px] flex-1">
									<label
										for="member-email-{org.id}"
										class="mb-1 block text-xs font-medium text-text-tertiary"
										>Add member by email</label
									>
									<input
								...
								<div>
									<label
										for="member-role-{org.id}"
										class="mb-1 block text-xs font-medium text-text-tertiary">Role</label
									>
									<select
											...
											{#each org.members as member (member.id)}
												<tr>
													<td class="px-4 py-2 font-medium text-text-primary"
														>{member.user.name ?? 'Unnamed'}</td
													>
													<td class="px-4 py-2 text-text-secondary">{member.user.email}</td>
													<td class="px-4 py-2">
														<span
															class="rounded bg-surface-subtle px-2 py-0.5 text-xs font-semibold text-text-secondary uppercase"
															>{member.user.role}</span
														>
													</td>
													<td class="px-4 py-2">
														<span
															class="rounded px-2 py-0.5 text-xs font-semibold uppercase {roleBadge(
																member.role
															)}">{member.role}</span
														>
													</td>
													<td class="px-4 py-2">
														<form method="post" action="?/removeMember">
															...
															<button
																type="submit"
																class="rounded border border-border-default px-3 py-1 text-xs font-medium text-error transition-colors hover:bg-error-muted"
																onclick={(e) => {
																	if (
																		!confirm(
																			`Remove ${member.user.name || member.user.email} from ${org.name}?`
																		)
																	) {
																		e.preventDefault();
																	}
