...
export const load: PageServerLoad = async (event) => {
	...
	if (cycle && currentWeek && isOnboardingComplete) {
		...
		if (reflectionsWithRatings.length > 0) {
			...
			const latestWeekReflections = reflectionsWithRatings.filter((r) => r.weekNumber === latestWeek);
			...
			const latestEffort = effortScores.length > 0
				? Number((effortScores.reduce((sum, s) => sum + s, 0) / effortScores.length).toFixed(1))
				: null;
			const latestPerformance = performanceScores.length > 0
				? Number((performanceScores.reduce((sum, s) => sum + s, 0) / performanceScores.length).toFixed(1))
				: null;
			...
			if (previousWeek) {
				const previousWeekReflections = cycle.reflections.filter((r) => r.weekNumber === previousWeek);
				...
				previousEffort = prevEffortScores.length > 0
					? Number((prevEffortScores.reduce((sum, s) => sum + s, 0) / prevEffortScores.length).toFixed(1))
					: null;
				previousPerformance = prevPerformanceScores.length > 0
					? Number((prevPerformanceScores.reduce((sum, s) => sum + s, 0) / prevPerformanceScores.length).toFixed(1))
					: null;
			}
			...
			myLastRatings = {
				...
				effortChange: latestEffort !== null && previousEffort !== null ? latestEffort - previousEffort : null,
				performanceChange: latestPerformance !== null && previousPerformance !== null ? latestPerformance - previousPerformance : null,
		...
		if (allFeedbacks.length > 0) {
			...
			const feedbacksByWeek = new Map<number, Array<{ effortScore: number | null; performanceScore: number | null }>>();
			...
			const latestStakeholderEffort = stakeholderEffortScores.length > 0
				? Number((stakeholderEffortScores.reduce((sum, s) => sum + s, 0) / stakeholderEffortScores.length).toFixed(1))
				: null;
			const latestStakeholderPerformance = stakeholderPerformanceScores.length > 0
				? Number((stakeholderPerformanceScores.reduce((sum, s) => sum + s, 0) / stakeholderPerformanceScores.length).toFixed(1))
				: null;

			// Get previous week stakeholder ratings
			const previousWeekWithFeedback = latestWeekWithFeedback > 1 ? latestWeekWithFeedback - 1 : null;
			...
			if (previousWeekWithFeedback && feedbacksByWeek.has(previousWeekWithFeedback)) {
				...
				previousStakeholderEffort = prevEffortScores.length > 0
					? Number((prevEffortScores.reduce((sum, s) => sum + s, 0) / prevEffortScores.length).toFixed(1))
					: null;
				previousStakeholderPerformance = prevPerformanceScores.length > 0
					? Number((prevPerformanceScores.reduce((sum, s) => sum + s, 0) / prevPerformanceScores.length).toFixed(1))
					: null;
			}
			...
			stakeholdersLastRatings = {
				...
				effortChange: latestStakeholderEffort !== null && previousStakeholderEffort !== null
					? latestStakeholderEffort - previousStakeholderEffort
					: null,
				performanceChange: latestStakeholderPerformance !== null && previousStakeholderPerformance !== null
