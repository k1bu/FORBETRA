...
<section class="mx-auto flex min-h-screen max-w-4xl flex-col gap-6 p-4 pb-12">
	...
	{/if}
	{#if draftRestored}
		<div class="fixed bottom-4 left-1/2 z-50 -translate-x-1/2 rounded-lg border border-success-muted bg-success-muted px-4 py-2 text-sm font-medium text-success transition-opacity">
			Draft restored
		</div>
	{/if}
	<header class="space-y-3 text-center">
		...
		<h1 class="text-3xl font-bold text-text-primary">Share feedback for {data.reflection.participantName}</h1>
		...
		<div class="mx-auto max-w-2xl space-y-6">
			<div class="rounded-2xl border border-accent/30 bg-surface-base p-8">
				...
				<div class="mb-6 rounded-xl border border-accent/30 bg-surface-raised p-5">
					<p class="mb-1 text-xs font-semibold uppercase tracking-wide text-accent">What they're working on</p>
				...
				<div class="mb-6 rounded-lg bg-accent-muted border border-accent/30 p-4">
				...
				<div class="slide-in-from-bottom-4 rounded-2xl border border-accent/30 bg-surface-base p-8">
					...
					<div class="grid gap-6 md:grid-cols-2">
						...
						<div class="rounded-xl border border-border-default bg-surface-raised p-6">
							...
							<div class="space-y-3">
								<div class="flex items-center justify-between rounded-lg border border-border-default bg-surface-subtle px-4 py-3">
		...
		<div class="mx-auto w-full max-w-2xl space-y-6">
			...
			{#if data.subgoals && data.subgoals.length > 0}
				<div class="rounded-xl border border-accent/30 bg-surface-base px-5 py-4">
					...
					<p class="mt-3 text-xs text-accent">Keep these in mind as you rate effort and performance below.</p>
				</div>
			{/if}
			...
			{#if data.previousRatings && (data.previousRatings.effortScore !== null || data.previousRatings.performanceScore !== null)}
				...
				<div class="rounded-xl border border-accent/30 bg-accent-muted p-4">
					<div class="mb-3">
						<p class="text-sm font-semibold text-accent">Your last ratings for {data.reflection.participantName}:</p>
					</div>
					<div class="flex gap-6 text-sm">
						{#if data.previousRatings.effortScore !== null}
							<div class="flex items-center gap-2">
								<span class="text-accent">Effort:</span>
								<span class="font-bold text-lg text-accent">{data.previousRatings.effortScore}</span>
							</div>
						{/if}
						{#if data.previousRatings.performanceScore !== null}
							<div class="flex items-center gap-2">
								<span class="text-accent">Performance:</span>
								<span class="font-bold text-lg text-accent">{data.previousRatings.performanceScore}</span>
							</div>
						{/if}
					</div>
