...
export function buildGapNarrativePrompt(gap: PerceptionGap): string {
	...
	return `The participant rated their ${gap.dimension} as ${gap.selfScore}/10. Their reviewers averaged ${gap.raterAvg.toFixed(1)}/10 — a gap of ${Math.abs(gap.gap).toFixed(1)} points (self is ${direction} reviewers).
