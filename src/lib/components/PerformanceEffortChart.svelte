<script lang="ts">
				...
				averages.push({
					...
					avgEffort:
						data.effortScores.length > 0
							? Number(
									(
										data.effortScores.reduce((sum, s) => sum + s, 0) / data.effortScores.length
									).toFixed(1)
								)
							: null,
					avgProgress:
						data.performanceScores.length > 0
							? Number(
									(
										data.performanceScores.reduce((sum, s) => sum + s, 0) /
	...
	const hasLimitedData = $derived(
		(() => {
			const dataWeeks = new SvelteSet<number>();
		...
		(() => {
			...
			if (!baselineWeek)
				return {
					...
					stakeholderEffort: null,
					stakeholderProgress: null
				};

			const baselineIndividual = individualData.find((d) => d.weekNumber === baselineWeek);
			const baselineStakeholder = stakeholderAveragesByWeek.find(
				(d) => d.weekNumber === baselineWeek
			);
	...
	const currentValues = $derived(
		(() => {
			...
			const individualEffortWeeks = individualData
				...
				.map((d) => d.weekNumber)
				.sort((a, b) => b - a);
			const latestIndividualEffort = individualEffortWeeks[0]
				? individualData.find((d) => d.weekNumber === individualEffortWeeks[0])
				: null;
			...
			const individualProgressWeeks = individualData
				...
				.map((d) => d.weekNumber)
				.sort((a, b) => b - a);
			const latestIndividualProgress = individualProgressWeeks[0]
				? individualData.find((d) => d.weekNumber === individualProgressWeeks[0])
				: null;
			...
			const stakeholderEffortWeeks = stakeholderAveragesByWeek
				...
				.map((d) => d.weekNumber)
				.sort((a, b) => b - a);
			const latestStakeholderEffort = stakeholderEffortWeeks[0]
				? stakeholderAveragesByWeek.find((d) => d.weekNumber === stakeholderEffortWeeks[0])
				: null;
			...
			const stakeholderProgressWeeks = stakeholderAveragesByWeek
				...
				.map((d) => d.weekNumber)
				.sort((a, b) => b - a);
			const latestStakeholderProgress = stakeholderProgressWeeks[0]
				? stakeholderAveragesByWeek.find((d) => d.weekNumber === stakeholderProgressWeeks[0])
				: null;
	...
	const stakeholderSummaryData = $derived(
		(() => {
			...
			const stakeholderDataWeeks = new SvelteSet<number>();
			...
			const uniqueStakeholders = new SvelteMap<string, string>();
			stakeholderData.forEach((d) => {
			...
			return stakeholderSummaries.sort((a, b) =>
		...
		if (performanceChartCanvas && performanceChartConfig) {
			...
			if (ctx) {
				performanceChartInstance = new Chart(
					ctx,
					performanceChartConfig as ChartConfiguration<'line'>
				);
			}
		}
	});
	...
	function selectAllStakeholders() {
		selectedStakeholderIds = new SvelteSet(stakeholders.map((s) => s.id));
	}

	function clearStakeholderFilter() {
		selectedStakeholderIds = new SvelteSet<string>();
	}
</script>
...
<div class="space-y-4">
	...
	<div class="rounded-xl border border-border-default bg-surface-raised p-4">
		...
		<div class="flex flex-wrap items-center gap-4 border-b border-border-default pb-3">
			<span class="text-xs font-semibold tracking-wide text-text-tertiary uppercase">Metrics:</span>
		...
		{#if stakeholders.length > 0 && stakeholdersWithData.length > 0}
			<div class="flex flex-wrap items-center gap-6 pt-3">
				...
				<div class="flex items-center gap-4">
					<span class="text-xs font-semibold tracking-wide text-text-tertiary uppercase">View:</span
					>
					<label class="flex items-center gap-2">
						<input
							type="checkbox"
			...
			<div class="overflow-x-auto rounded-xl border border-border-default bg-surface-raised">
				<table class="w-full text-sm">
					<thead class="bg-surface-raised">
						<tr>
							<th
								class="px-5 py-3 text-left text-xs font-semibold tracking-wide text-text-secondary uppercase"
								>Metric</th
							>
							<th
								class="px-5 py-3 text-center text-xs font-semibold tracking-wide text-text-secondary uppercase"
							>
								{baseline.individualEffort !== null || baseline.individualProgress !== null
									? allWeeks.find((w) => w === 0) !== undefined
										? 'Initial'
										: 'Week 1'
									: 'Week 1'}
							</th>
					...
					<tbody class="divide-y divide-border-default">
						{#if showEffort}
							...
							<tr class="bg-accent-muted/30">
								...
								<td
									class="px-5 py-3 text-center font-semibold {summaryData.individualEffort.delta !==
										null && summaryData.individualEffort.delta >= 0
										? 'text-success'
										: 'text-error'}"
								>
								...
								<td
									class="px-5 py-3 text-center font-semibold {summaryData.individualEffort
										.percentChange !== null && summaryData.individualEffort.percentChange >= 0
										? 'text-success'
										: 'text-error'}"
								>
							...
							{#if stakeholderAveragesByWeek.length > 0}
								...
								<tr class="bg-surface-subtle/50">
									...
									<td
										class="px-5 py-3 text-center font-semibold {summaryData.stakeholderEffort
											.delta !== null && summaryData.stakeholderEffort.delta >= 0
											? 'text-success'
											: 'text-error'}"
									>
									...
									<td
										class="px-5 py-3 text-center font-semibold {summaryData.stakeholderEffort
											.percentChange !== null && summaryData.stakeholderEffort.percentChange >= 0
											? 'text-success'
											: 'text-error'}"
									>
									...
									{#each stakeholderSummaryData as stakeholder (stakeholder.stakeholderId)}
										{#if stakeholder.effort.week1 !== null || stakeholder.effort.current !== null}
											<tr class="bg-surface-raised/50">
												...
												<td
													class="px-5 py-2.5 text-center text-sm font-medium {stakeholder.effort
														.delta !== null && stakeholder.effort.delta >= 0
														? 'text-success'
														: 'text-error'}"
												>
												...
												<td
													class="px-5 py-2.5 text-center text-sm font-medium {stakeholder.effort
														.percentChange !== null && stakeholder.effort.percentChange >= 0
														? 'text-success'
														: 'text-error'}"
												>
						...
						{#if showPerformance}
							...
							<tr class="bg-warning-muted/30">
								...
								<td
									class="px-5 py-3 text-center font-semibold {summaryData.individualProgress
										.delta !== null && summaryData.individualProgress.delta >= 0
										? 'text-success'
										: 'text-error'}"
								>
								...
								<td
									class="px-5 py-3 text-center font-semibold {summaryData.individualProgress
										.percentChange !== null && summaryData.individualProgress.percentChange >= 0
										? 'text-success'
										: 'text-error'}"
								>
							...
							{#if stakeholderAveragesByWeek.length > 0}
								...
								<tr class="bg-surface-subtle/50">
									...
									<td
										class="px-5 py-3 text-center font-semibold {summaryData.stakeholderProgress
											.delta !== null && summaryData.stakeholderProgress.delta >= 0
											? 'text-success'
											: 'text-error'}"
									>
									...
									<td
										class="px-5 py-3 text-center font-semibold {summaryData.stakeholderProgress
											.percentChange !== null && summaryData.stakeholderProgress.percentChange >= 0
											? 'text-success'
											: 'text-error'}"
									>
									...
									{#each stakeholderSummaryData as stakeholder (stakeholder.stakeholderId)}
										{#if stakeholder.performance.week1 !== null || stakeholder.performance.current !== null}
											<tr class="bg-surface-raised/50">
												...
												<td
													class="px-5 py-2.5 text-center text-sm font-medium {stakeholder
														.performance.delta !== null && stakeholder.performance.delta >= 0
														? 'text-success'
														: 'text-error'}"
												>
												...
												<td
													...
													stakeholder.performance.percentChange >= 0
														? 'text-success'
														: 'text-error'}"
												>
